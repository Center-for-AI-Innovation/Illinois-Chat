...
Which Sim blocks users may place in a workflow is restricted by `ALLOWED_INTEGRATIONS`, whose full value ships as the default in `infra/docker/docker-compose.sim.yaml`; the builder-facing view of that list is section 5 of the Sim access doc linked above. Four properties of this variable are easy to get wrong. It is enforced on its own, with no access-control or enterprise licence needed. **A variable Sim receives as empty means unrestricted, not restricted**, which is why the list ships as the compose default rather than living only in `.env`; leaving `SIM_ALLOWED_INTEGRATIONS` blank or unset there is safe, because the `:-` default applies, but deleting the compose key removes the restriction entirely. A non-empty value in `.env` replaces the shipped default outright rather than adding to it. Ids are matched exactly and case-insensitively with no version resolution on the pinned image, so `slack` does not permit `slack_v2`. Core blocks are not exempt, so omitting `agent` or `function` breaks every workflow. Enforcement happens when a block executes, not when a workflow is saved, so a workflow containing a denied block can still be saved or imported and will fail only on a run; a denied vendor attached as an agent *tool* is dropped silently and the agent runs without it. MCP servers are not restricted by host in this release. To roll the whitelist back, comment the `ALLOWED_INTEGRATIONS` key out of the compose file and recreate the `simstudio` service - clearing the `.env` variable does nothing, because the compose default takes over.
