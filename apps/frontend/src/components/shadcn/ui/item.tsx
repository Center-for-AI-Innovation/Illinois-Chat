...
function ItemDescription({ className, ...props }: React.ComponentProps<'p'>) {
  return (
    <p
      ...
      className={cn(
        'line-clamp-2 text-left text-sm font-normal leading-normal text-muted-foreground group-data-[size=xs]/item:text-xs [&>a:hover]:text-primary [&>a]:underline [&>a]:underline-offset-4',
        className,
      )}
      {...props}
    />
