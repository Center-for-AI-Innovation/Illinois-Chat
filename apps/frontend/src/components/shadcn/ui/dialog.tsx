  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          'bg-popover text-popover-foreground ring-foreground/10 data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 fixed top-1/2 left-1/2 z-50 grid max-h-[90vh] w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-6 overflow-y-auto rounded-xl p-6 text-sm ring-1 outline-hidden duration-100 sm:max-w-md',
          className,
        )}
        {...props}
      >
...
function DialogDescription({
  ...
  return (
    <DialogPrimitive.Description
      ...
      className={cn(
        '*:[a]:hover:text-foreground text-muted-foreground text-sm *:[a]:underline *:[a]:underline-offset-[3px]',
        className,
      )}
      {...props}
    />
