...
export const TemperatureSlider: FC<Props> = ({
  ...
  const handleChange = (value: number | readonly number[]) => {
    const next = Array.isArray(value) ? (value[0] ?? temperature) : (value as number)
    setTemperature(next)
    onChangeTemperature(next)
  }
  ...
  return (
    <div className="flex flex-col">
      {isSmallScreen ? (
        <h5
          className={`heading-h5 px-4 pt-4 ${montserrat_heading.variable} font-montserratHeading rounded-lg bg-(--modal-dark) p-4 text-(--modal-text) md:rounded-lg`}
        >
          {label}
        </h5>
      ) : (
        <h4
          className={`heading-h4 px-4 pt-4 ${montserrat_heading.variable} font-montserratHeading rounded-lg bg-(--modal-dark) p-4 text-(--modal-text) md:rounded-lg`}
        >
          {label}
        </h4>
      )}
      <div className="mx-6 my-4 flex flex-col">
        ...
        <Slider
          ...
          className="m-2"
          markLabelClassName={`text-(--foreground-faded) ${
