...
class MinioClient:
  ...
  def __init__(self):
    ...
    self.client = Minio(endpoint=os.environ['MINIO_ENDPOINT'],
                        access_key=os.environ['MINIO_ACCESS_KEY'],
                        secret_key=os.environ['MINIO_SECRET_KEY'],
                        secure=os.environ.get('MINIO_SECURE', 'false').lower() == 'true')
  ...
  def list_pubmed_pdfs(self) -> List[str]:
    ...
    return [obj.object_name for obj in self.client.list_objects(BUCKET_NAME, recursive=True) if obj.object_name.lower().endswith('.pdf')]
...
def main():
  ...
  with ProcessPoolExecutor(max_workers=workers) as executor:
    future_to_file = {executor.submit(process_pudmed_file, fn, processed): fn for fn in remaining}
