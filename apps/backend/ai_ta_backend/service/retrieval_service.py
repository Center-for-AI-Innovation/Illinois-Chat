...
from ai_ta_backend.database.sql import ModelUsage
from ai_ta_backend.database.sql import ProjectStats
from ai_ta_backend.database.sql import WeeklyMetric
...
def _parse_allowed_embedding_providers() -> tuple[str, ...]:
  ...
  if not parsed:
    raise ValueError("ALLOWED_EMBEDDING_PROVIDERS is set but parses to an empty list. "
...
class RetrievalService:
  ...
  async def getTopContexts(
    ...
    try:
      ...
      try:
        embedding_client, query_instruction = self._resolve_embedding_client(course_name)
      ...
      with self.thread_pool_executor as executor:
        ...
        tasks = [
            loop.run_in_executor(executor, project_sql_db.getDisabledDocGroups, course_name),
            loop.run_in_executor(executor, project_sql_db.getPublicDocGroups, course_name),
      ...
      user_query_embedding, openai_embedding_latency = embed_result
      ...
      found_docs: list[Document] = self.vector_search(
          ...
          openai_embedding_latency=openai_embedding_latency,
      )
  ...
  def delete_from_qdrant(self, identifier_key: str, identifier_value: str, course_name: str):
    ...
    try:
      ...
      if engine_kind == "qdrant":
        collection_name = vdb.qdrant_config.get("default_collection", os.environ.get("QDRANT_COLLECTION_NAME", ""))
      else:
        # pgvector ignores collection_name; pass a sentinel for logs only.
        collection_name = os.environ.get("QDRANT_COLLECTION_NAME", "")

      response = vdb.delete_data(collection_name, identifier_key, identifier_value)
  ...
  def delete_from_nomic_and_database(self, course_name: str, identifier_key: str, identifier_value: str):
    try:
      ...
      project_sql_db = self.conn_manager.get_documents_sql_db(course_name)
      response = project_sql_db.deleteMaterialsForCourseAndKeyAndValue(course_name, identifier_key, identifier_value)
    except Exception as e:
  ...
  def vector_search(
    ...
    if engine_kind == "qdrant":
      search_filter = vdb._create_search_filter(course_name, doc_groups, disabled_doc_groups, public_doc_groups)
      if conversation_id:
        ...
        search_results = vdb.execute_search(combined_filter, user_query_embedding, course_name, top_n)
      else:
        search_results = vdb.execute_search(search_filter, user_query_embedding, course_name, top_n)
    else:
    ...
    qdrant_latency_sec = time.monotonic() - start_time_vector_search
    ...
    self._capture_search_succeeded_event(
        search_query,
        course_name,
        search_results,
        qdrant_latency_sec=qdrant_latency_sec,
        openai_embedding_latency=openai_embedding_latency,
    )
    time_for_capture_search_succeeded_event = (time.monotonic() - start_time_capture_search_succeeded_event)

    print(f"Runtime for embedding query: {openai_embedding_latency:.2f} seconds\n"
          f"Runtime for vector search: {qdrant_latency_sec:.2f} seconds\n"
  ...
  def _embed_query_and_measure_latency(self, search_query, embedding_client, query_instruction: str | None = None):
    ...
    return user_query_embedding, openai_embedding_latency
  ...
  def _resolve_embedding_client(self, project_name: str):
    """Resolve the embedding client and query instruction for a project.
        ...
        """
    embedding_cfg = self.conn_manager._get_decrypted_field(project_name, "embedding_config")
    if not embedding_cfg:
      qdrant_cfg = self.conn_manager._get_decrypted_field(project_name, "qdrant_config")
    ...
    query_instruction = embedding_cfg.get("query_instruction", self.qwen_query_instruction)

    if provider == "ollama":
      base_url = embedding_cfg.get("base_url") or os.environ.get("OLLAMA_SERVER_URL")
      if not base_url:
        raise ValueError("embedding_config provider='ollama' requires base_url "
  ...
  def getConversationStats(self, course_name: str, from_date: str = "", to_date: str = ""):
    ...
    try:
      ...
      project_sql_db = self.conn_manager.get_sql_db()
      conversations, total_count = (project_sql_db.getConversationsCreatedAtByCourse(course_name, from_date, to_date))
  ...
  def _store_conversation_content(
    ...
    try:
      ...
      if ids:
        ...
        vdb.upsert_main_collection(ids=ids, vectors=vectors, payloads=payloads, wait=True)
