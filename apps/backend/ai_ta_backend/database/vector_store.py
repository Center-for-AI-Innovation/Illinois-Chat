...
class PgVectorStore:
  ...
  def delete_by_filter(self, key: str, value: str) -> int:
    ...
    if key not in ("course_name", "s3_path", "url", "conversation_id", "readable_filename"):
      raise ValueError(f"PgVectorStore.delete_by_filter: unsupported key {key!r}. "
  ...
  def search(
    ...
    for pdg in public_doc_groups:
      ...
      if restrict_doc_groups and pdg_name not in doc_groups:
        continue
      should_clauses.append("(course_name = %s AND doc_groups @> %s::jsonb)")
    ...
    if disabled_doc_groups:
      overlap, overlap_params = _doc_groups_overlap_sql("doc_groups", disabled_doc_groups)
      if overlap:
    ...
    if conversation_id:
      where_clauses.append(f"({regular_sql} OR conversation_id = %s)")
