...
def _host_only(method):
  ...
  def wrapper(self, *args, **kwargs):
    if getattr(self, "is_external", False):
      raise RuntimeError(f"SQLDatabase.{method.__name__} is host-only and must not be "
...
class SQLDatabase:
  ...
  def __init__(self, engine=None) -> None:
    ...
    DB_CONFIGS = {'sqlite': ['SQLITE_DB_NAME'], 'postgres': ['POSTGRES_USERNAME', 'POSTGRES_PASSWORD', 'POSTGRES_ENDPOINT']}
  ...
  def getAllMaterialsForCourse(self, course_name: str):
    query = (select(models.Document.course_name, models.Document.s3_path, models.Document.readable_filename, models.Document.url,
                    models.Document.base_url).where(models.Document.course_name == course_name))
  ...
  def getMaterialsForCourseAndS3Path(self, course_name: str, s3_path: str):
    query = (select(models.Document).where(models.Document.s3_path == s3_path).where(models.Document.course_name == course_name))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(doc) for doc in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getMatrialsForCourseAndUrl(self, course_name: str, url: str):
    query = (select(models.Document).where(models.Document.url == url).where(models.Document.course_name == course_name))
  ...
  def getMaterialsForCourseAndKeyAndValue(self, course_name: str, key: str, value: str):
    query = (select(
        models.Document.id, models.Document.s3_path,
        models.Document.contexts).where(getattr(models.Document, key) == value).where(models.Document.course_name == course_name))
  ...
  def deleteMaterialsForCourseAndKeyAndValue(self, course_name: str, key: str, value: str):
    delete_stmt = (delete(models.Document).where(getattr(models.Document, key) == value).where(models.Document.course_name == course_name))
  ...
  def deleteMaterialsForCourseAndS3Path(self, course_name: str, s3_path: str):
    delete_stmt = (delete(models.Document).where(models.Document.s3_path == s3_path).where(models.Document.course_name == course_name))
  ...
  def getProjectsMapForCourse(self, course_name: str):
    query = (select(models.Project.doc_map_id).where(models.Project.course_name == course_name))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(doc) for doc in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getDocumentsBetweenDates(self, course_name: str, from_date: str, to_date: str):
    query = (select(models.Document).where(models.Document.course_name == course_name))
  ...
  def getConversationsBetweenDates(self, course_name: str, from_date: str, to_date: str):
    query = (select(models.LlmConvoMonitor).where(models.LlmConvoMonitor.course_name == course_name))
    from_date = to_utc_datetime(from_date)
    to_date = to_utc_datetime(to_date, end_of_day=True)
    if from_date:
  ...
  def getAllFromTableForDownloadType(self, course_name: str, download_type: str, first_id: int):
    if download_type == 'documents':
      query = (select(models.Document).where(models.Document.course_name == course_name).where(models.Document.id >= first_id).order_by(
          models.Document.id.asc()).limit(100))
    else:
      query = (select(models.LlmConvoMonitor).where(models.LlmConvoMonitor.course_name == course_name).where(
          models.LlmConvoMonitor.id >= first_id).order_by(models.LlmConvoMonitor.id.asc()).limit(100))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(doc) for doc in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getAllConversationsBetweenIds(self, course_name: str, first_id: int, last_id: int, limit: int = 50):
    ...
    query = (query.order_by(models.LlmConvoMonitor.id.asc()).limit(limit))
  ...
  def getDocsForIdsGte(self, course_name: str, first_id: int, fields: str = "*", limit: int = 100):
    if fields != "*":
      ...
      query = (select(models.Document).where(models.Document.course_name == course_name).where(models.Document.id >= first_id).order_by(
          models.Document.id.asc()).limit(limit))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(r) for r in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getProjectByName(self, project_name: str):
    ...
    query = (select(models.Project).where(models.Project.course_name == project_name))
    with self.get_session() as session:
  ...
  def getLLMConvo(self):
    query = (select(models.LlmConvoMonitor.convo))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(r) for r in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getAllFromLLMConvoMonitor(self, course_name: str):
    query = (select(models.LlmConvoMonitor).where(models.LlmConvoMonitor.course_name == course_name).order_by(
        models.LlmConvoMonitor.id.asc()))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(r) for r in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getCountFromLLMConvoMonitor(self, course_name: str, last_id: int):
    if last_id == 0:
      query = (select(models.LlmConvoMonitor.id).where(models.LlmConvoMonitor.course_name == course_name).order_by(
          models.LlmConvoMonitor.id.asc()))
    else:
      query = (select(models.LlmConvoMonitor.id).where(models.LlmConvoMonitor.course_name == course_name).where(
          models.LlmConvoMonitor.id > last_id).order_by(models.LlmConvoMonitor.id.asc()))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(r) for r in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getCountFromDocuments(self, course_name: str, last_id: int):
    if last_id == 0:
      query = (select(models.Document.id).where(models.Document.course_name == course_name).order_by(models.Document.id.asc()))
    else:
      query = (select(models.Document.id).where(models.Document.course_name == course_name).where(models.Document.id > last_id).order_by(
  ...
  def getConvoMapFromProjects(self, course_name: str):
    query = (select(models.Project).where(models.Project.course_name == course_name))
    with self.get_session() as session:
      result = session.execute(query).mappings().all()
      response = DatabaseResponse(data=result, count=len(result)).to_dict()
  ...
  def updateProjects(self, course_name: str, data: dict):
    query = (select(models.Project).where(models.Project.course_name == course_name).update(data))
    with self.get_session() as session:
  ...
  def getLatestWorkflowId(self):
    query = (select(models.N8nWorkflows))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(doc) for doc in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def deleteLatestWorkflowId(self, id: int):
    query = (delete(models.N8nWorkflows).where(models.N8nWorkflows.latest_workflow_id == id))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(doc) for doc in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def unlockWorkflow(self, id: int):
    query = (update(models.N8nWorkflows).where(models.N8nWorkflows.latest_workflow_id == id).values(is_locked=False))
    with self.get_session() as session:
      result = session.execute(query)
  ...
  def getConversation(self, course_name: str, key: str, value: str):
    query = (select(models.LlmConvoMonitor).where(getattr(models.LlmConvoMonitor, key) == value).where(
  ...
  def getDisabledDocGroups(self, course_name: str):
    query = (select(models.DocGroup.name).where(models.DocGroup.course_name == course_name).where(models.DocGroup.enabled == False))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(doc) for doc in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getPublicDocGroups(self, course_name: str):
    query = (select(models.DocGroup.name, models.DocGroup.course_name, models.DocGroup.enabled, models.DocGroup.private,
                    models.DocGroup.doc_count).where(models.DocGroup.course_name == course_name))
  ...
  def getAllConversationsForUserAndProject(self, user_email: str, project_name: str, curr_count: int = 0):
    ...
    conv_page = (select(models.Conversations).where(C.user_email == user_email, C.project_name == project_name).order_by(
        models.Conversations.updated_at.desc()).limit(500).offset(curr_count).subquery())
    ...
    msg_obj = func.jsonb_build_object(
        ...
        M.latest_system_message,
        'final_prompt_engineered_message',
        M.final_prompt_engineered_message,
        'response_time_sec',
        M.response_time_sec,
        'content_text',
        M.content_text,
    ...
    messages_agg = func.coalesce(func.jsonb_agg(msg_obj.op("ORDER BY")(M.created_at)).filter(M.id.isnot(None)),
                                 func.cast('[]', JSONB)).label("messages")

    query = (select(CP.id.label("id"), CP.name.label("name"), CP.model, CP.prompt, CP.temperature, CP.user_email, CP.project_name,
                    CP.created_at, CP.updated_at, CP.folder_id,
                    messages_agg).select_from(conv_page).join(M, M.conversation_id == CP.id,
                                                              isouter=True).group_by(CP.id, CP.name, CP.model, CP.prompt, CP.temperature,
                                                                                     CP.user_email, CP.project_name, CP.created_at,
                                                                                     CP.updated_at,
                                                                                     CP.folder_id).order_by(desc(CP.updated_at)))
  ...
  def getPreAssignedAPIKeys(self, email: str):
    query = (select(models.PreAuthAPIKeys).where(models.PreAuthAPIKeys.emails.contains([email])))
    with self.get_session() as session:
      result = session.execute(query).scalars().all()
      data = [orm_to_dict(doc) for doc in result]
      response = DatabaseResponse(data=data, count=len(result)).to_dict()
  ...
  def getConversationsCreatedAtByCourse(self, course_name: str, from_date: str = "", to_date: str = ""):
    try:
      query = (select(models.LlmConvoMonitor.created_at).where(models.LlmConvoMonitor.course_name == course_name))
  ...
  def getProjectStats(self, project_name: str) -> ProjectStats:
    try:
      query = (select(models.ProjectStats.total_messages, models.ProjectStats.total_conversations,
                      models.ProjectStats.unique_users).where(models.ProjectStats.project_name == project_name))
      with self.get_session() as session:
        result = session.execute(query).mappings().all()
        response = DatabaseResponse(data=result, count=len(result)).to_dict()
  ...
  def getAllProjects(self):
    query = (select(models.Project.course_name, models.Project.doc_map_id, models.Project.convo_map_id, models.Project.last_uploaded_doc_id,
  ...
  def getProjectsWithConvoMaps(self):
    query = (select(models.Project.course_name, models.Project.convo_map_id, models.Project.last_uploaded_doc_id,
                    models.Project.last_uploaded_convo_id).where(models.Project.convo_map_id is not None))
    with self.get_session() as session:
      result = session.execute(query).mappings().all()
      response = DatabaseResponse(data=result, count=len(result)).to_dict()
  ...
  def getProjectsWithDocMaps(self):
    query = (select(models.Project.course_name, models.Project.doc_map_id, models.Project.last_uploaded_doc_id,
                    models.Project.document_map_index).where(models.Project.doc_map_id is not None))
    with self.get_session() as session:
      result = session.execute(query).mappings().all()
      response = DatabaseResponse(data=result, count=len(result)).to_dict()
  ...
  def getMessagesFromConvoID(self, convo_id):
    query = (select(models.Messages).where(models.Messages.conversation_id == convo_id).limit(500))
    with self.get_session() as session:
      result = session.execute(query).mappings().all()
      response = DatabaseResponse(data=result, count=len(result)).to_dict()
  ...
  def updateMessageFromLlmMonitor(self, message_id, llm_monitor_tags):
    query = (select(models.Message).where(models.Message.id == message_id).update({"llm_monitor_tags": llm_monitor_tags}))
    with self.get_session() as session:
      result = session.execute(query).mappings().all()
      response = DatabaseResponse(data=result, count=len(result)).to_dict()
  ...
  def getExternalConnection(self, project_name: str):
    query = (select(models.ProjectExternalConnection).where(models.ProjectExternalConnection.project_name == project_name).where(
        models.ProjectExternalConnection.is_active == True))
    with self.get_session() as session:
      result = session.execute(query).scalars().first()
      return orm_to_dict(result)
