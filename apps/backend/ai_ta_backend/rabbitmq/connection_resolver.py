...
class ResolvedConnections:
  ...
  def has_overrides(self) -> bool:
    return any([self.qdrant_client, self.s3_client, self.documents_sql_engine, self.pgvector_store, self.embedding_provider])
...
class WorkerConnectionResolver:
  ...
  def resolve(self, project_name: str) -> ResolvedConnections:
    ...
    if db_config:
      ...
      if result.engine_kind == 'pgvector':
        result.pgvector_store = self._get_or_create_pgvector_store(project_name, engine)
  ...
  def _resolve_embedding(self, result: ResolvedConnections, project_name: str, emb_cfg: dict) -> None:
  ...
  def _get_or_create_pgvector_store(self, project_name: str, engine: SAEngine) -> PgVectorStore:
