...
class Ingest:
  ...
  def __init__(self):
    ...
    self.openai_api_base = os.getenv('EMBEDDING_API_BASE') + "/embeddings" if os.getenv(
        'EMBEDDING_API_BASE') else 'https://api.openai.com/v1/embeddings'
  ...
  def initialize_resources(self,
    ...
    if s3_client is not None:
      ...
      self.s3_client = boto3.client('s3', endpoint_url=self.minio_url, config=Config(s3={'addressing_style': 'path'}))
    ...
    if self.embedding_model == 'text-embedding-ada-002':
      self.vectorstore = Qdrant(client=self.qdrant_client,
                                ...
                                embeddings=OpenAIEmbeddings(openai_api_type='openai',
                                                            openai_api_key=self.ncsa_hosted_api_key,
  ...
  def main_ingest(self, job_id: str, _resolved_connections=None, **inputs: Dict[str | List[str], Any]):
    ...
    try:
      ...
      if success_fail_dict['failure_ingest']:
        ...
        error_msg = (failure.get('error', str(failure)) if isinstance(failure, dict) else str(failure))
  ...
  def run_ingest(self, course_name, s3_paths, base_url, url, readable_filename, content, document_groups, force_embeddings=False):
    ...
    if content:
      ...
      return self.bulk_ingest(course_name,
                              s3_paths,
                              base_url=base_url,
                              url=url,
                              groups=document_groups,
  ...
  def bulk_ingest(self, course_name: str, s3_paths: Union[str, List[str]], force_embeddings: bool,
    ...
    try:
      ...
      for s3_path in s3_paths:
        ...
        if file_extension in file_ingest_methods:
          ...
          try:
            ...
            success_status['failure_ingest'] = {'s3_path': s3_path, 'error': err_msg}
            if self.posthog:
      ...
      try:
        error_s3_path = s3_path if 's3_path' in locals() else (
            s3_paths[0] if isinstance(s3_paths, list) and len(s3_paths) > 0 else s3_paths)
      except (NameError, TypeError, IndexError):
        error_s3_path = str(s3_paths) if s3_paths else "unknown"
      success_status['failure_ingest'] = {'s3_path': error_s3_path, 'error': f"MAJOR ERROR DURING INGEST: {err}"}
      if self.posthog:
        self.posthog.capture('distinct_id_of_the_user',
                             event='ingest_failure',
                             properties={
  ...
  def split_and_upload(self, texts: List[str], metadatas: List[Dict[str, Any]], force_embeddings: bool, **kwargs):
    ...
    try:
      ...
      if self.embedding_provider == "ollama":
        ...
        embeddings_dict: dict[str, List[float]] = {text: vector for text, vector in zip(texts_to_embed, vectors)}
      else:
      ...
      if self.qdrant_client is not None:
        ...
        for context in contexts:
          ...
          if len(ids_batch) >= self.upsert_batch_size:
            try:
              self.pgvector_store.upsert_batch(ids=ids_batch, vectors=vectors_batch, payloads=payloads_batch, wait=False)
            except Exception as e:
              logging.warning("pgvector batch upsert error (continuing): %s", e)
            ids_batch, vectors_batch, payloads_batch = [], [], []
        if ids_batch:
          try:
            self.pgvector_store.upsert_batch(ids=ids_batch, vectors=vectors_batch, payloads=payloads_batch, wait=True)
          except Exception as e:
            logging.warning("Final batch upsert encountered an error (continuing): %s", e)
  ...
  def check_for_duplicates(self, texts: List[Dict], metadatas: List[Dict[str, Any]], force_embeddings: bool) -> bool:
    ...
    if incoming_s3_path:
      ...
      logging.info(f"No. of S3 path based records retrieved: {len(contents)}")  # multiple records can be retrieved: 3.pdf and 453.pdf
    elif url:
  ...
  def ingest_single_web_text(self, course_name: str, base_url: str, url: str, content: str, readable_filename: str, force_embeddings: bool,
                             **kwargs) -> Dict[str, None | str | Dict[str, str]]:
    """Crawlee integration"""
    if self.posthog:
      self.posthog.capture('distinct_id_of_the_user',
  ...
  def _ingest_single_txt(self, s3_path: str, course_name: str, force_embeddings: bool, **kwargs) -> str:
    ...
    try:
      ...
      success_or_failure = self.split_and_upload(texts=text, metadatas=metadatas, force_embeddings=force_embeddings, **kwargs)
      return success_or_failure
    except Exception as e:
