...
class Ingest:
  ...
  def initialize_resources(self):
    ...
    if self.qdrant_api_key and self.qdrant_url:
      ...
      existing_collections = [collection.name for collection in self.qdrant_client.get_collections().collections]
      if self.qdrant_collection_name not in existing_collections:
        logging.info(f"Creating collection {self.qdrant_collection_name}")
        self.qdrant_client.create_collection(collection_name=self.qdrant_collection_name,
                                             vectors_config={
                                                 "size": 4096,
                                                 "distance": "Cosine"
                                             })
