...
class IngestCanvas:
  ...
  def auto_accept_enrollments(self, target_course_id):
    try:
      ...
      user_response = requests.get(f"{self.canvas_url}/api/v1/users/self", headers=self.headers)
  ...
  def ingest_course_content(self, canvas_course_id: int, course_name: str, content_ingest_dict: Dict[str, bool] = None) -> List[str]:
    """
  ...
  def download_pages(self, dest_folder: str, api_path: str) -> str:
    ...
    try:
      ...
      for page in pages:
        ...
        if page['html_url'] != '':
          page_content_request = requests.get(f"{api_path}/pages/{page['page_id']}", headers=self.headers)
