...
try:
  from ai_ta_backend.rabbitmq.connection_resolver import \
...
class Queue:
  ...
  def is_connected(self):
    return (hasattr(self, 'connection') and self.connection.is_open and hasattr(self, 'channel') and self.channel.is_open)
  ...
  def addJobToIngestQueue(self, inputs, queue_name=None):
    ...
    doc_progress_payload = models.DocumentsInProgress(s3_path=s3_path_value,
    ...
    status_db = (SQLAlchemyIngestDB(engine=resolved.documents_sql_engine) if resolved.documents_sql_engine else sql_session)
    ...
    # RMQ message second
    message = {'job_id': new_job_id, 'status': 'queued', 'inputs': inputs}
