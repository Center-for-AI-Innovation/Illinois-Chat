...
class Worker:
  ...
  def process_job(self, channel, method, header, body):
    ...
    try:
      ...
      status_db = (SQLAlchemyIngestDB(engine=resolved.documents_sql_engine) if resolved.documents_sql_engine else self.sql_session)
      ...
      if retry_count < MAX_JOB_RETRIES:
        ...
        self.connect()
        self.channel.basic_publish(exchange='', routing_key=self.rabbitmq_queue, properties=properties, body=json.dumps(content))
      else:
  ...
  def listen_for_jobs(self):
    ...
    self.consumer = self.channel.basic_consume(queue=self.rabbitmq_queue, on_message_callback=self.process_job, auto_ack=False)
