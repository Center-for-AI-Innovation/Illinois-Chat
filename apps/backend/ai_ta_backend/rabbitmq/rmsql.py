...
class SQLAlchemyIngestDB:
  ...
  def fetch_document_in_progress(self, job_id: str) -> dict:
    with self.get_session() as session:
      result = session.query(models.DocumentsInProgress).filter(models.DocumentsInProgress.beam_task_id == job_id).first()
    ...
    with self.get_session() as session:
      try:
        result = session.execute(
            text("SELECT * FROM add_document_to_group_url(:p_course_name, :p_s3_path, :p_url, :p_readable_filename, :p_doc_groups)"),
            params)
  ...
  def add_document_to_group(self, contexts, groups):
    ...
    with self.get_session() as session:
      try:
        result = session.execute(
            text("SELECT * FROM add_document_to_group(:p_course_name, :p_s3_path, :p_url, :p_readable_filename, :p_doc_groups)"), params)
  ...
  def get_like_docs_by_s3_path(self, course_name, original_filename):
    query = (select(models.Document.id, models.Document.contexts,
                    models.Document.s3_path).where(models.Document.course_name == course_name).where(
                        models.Document.s3_path.like(f"%{original_filename}%")).order_by(desc(models.Document.id)))
  ...
  def get_like_docs_by_url(self, course_name, url):
    query = (select(models.Document.id,
                    models.Document.contexts, models.Document.url).where(models.Document.course_name == course_name).where(
                        models.Document.url.like(f"%{url}%")).order_by(desc(models.Document.id)))
    with self.get_session() as session:
      result = session.execute(query).mappings().all()
      response = DatabaseResponse(data=result, count=len(result)).to_dict()
    return response
  ...
  def delete_document_by_url(self, course_name: str, url: str):
    delete_stmt = (delete(models.Document).where(models.Document.url == url).where(models.Document.course_name == course_name))
    with self.get_session() as session:
      result = session.execute(delete_stmt)
      return result.rowcount  # Number of rows deleted
