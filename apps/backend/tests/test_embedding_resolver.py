...
from ai_ta_backend.service.retrieval_service import ALLOWED_EMBEDDING_PROVIDERS
...
def _build_service(embedding_client) -> RetrievalService:
  ...
  svc._resolve_embedding_client = MagicMock(return_value=(embedding_client, "Given a query, retrieve relevant passages"))
  return svc
...
def test_store_conversation_content_uses_embed_documents_not_embed_query():
  ...
  client.embed_query.side_effect = AssertionError("embed_query must not be called on the ingest path")
  ...
  for call in client.embed_documents.call_args_list:
    ...
    for text in texts:
      assert not text.startswith("Instruct:"), (f"Document chunk was prefixed with Qwen query instruction: {text!r}")
...
def test_resolve_embedding_client_rejects_unknown_provider(monkeypatch):
  ...
  monkeypatch.setattr(retrieval_service, "ALLOWED_EMBEDDING_PROVIDERS", ("openai",))
...
def test_resolve_embedding_client_accepts_allowed_provider(monkeypatch):
  ...
  monkeypatch.setattr(retrieval_service, "ALLOWED_EMBEDDING_PROVIDERS", ("openai", "ollama"))
  ...
  } if f == "embedding_config" else None)
  svc.conn_manager = conn_manager
