...
function createS3Client(): S3Client {
  ...
  const baseConfig: any = region ? { region } : {};
...
export async function uploadPdfToS3(
  ...
  const filename =
    nameWithoutExtension.replace(/[^a-zA-Z0-9]/g, "-") + extension;
  ...
  if (!contentType.includes("application/pdf") && head !== "%PDF-") {
    console.warn(
      `SKIP-PDF (not-a-pdf, content-type=${contentType || "n/a"}): url=${url}`,
    );
    return null;
  }
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.name === "NotFound") {
        ...
        try {
          await s3Client.send(
            new CreateBucketCommand({ Bucket: s3BucketName }),
          );
  ...
  try {
    await s3Client.send(
      new HeadObjectCommand({ Bucket: s3BucketName, Key: s3Key }),
    );
