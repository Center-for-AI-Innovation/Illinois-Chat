...
try:
  ...
  raise SystemExit("[error] python-dotenv is required. Install with:\n"
...
def _build_config_from_env(kind: str) -> dict:
  ...
  def require(field: str, var: str) -> str:
    value = _env(var)
    if value is None:
      raise SystemExit(f"[error] {kind} config requires {var} — set it in {ENV_FILE} "
  ...
  if kind == "s3":
    ...
    config: dict[str, Any] = {
        ...
        "aws_secret_access_key": require("aws_secret_access_key", "EXT_S3_SECRET_ACCESS_KEY"),
    }
    for field, var in (
...
def _require_project(name: str) -> str:
  if not name:
    raise SystemExit("[error] no project name given — pass it on the command line or "
